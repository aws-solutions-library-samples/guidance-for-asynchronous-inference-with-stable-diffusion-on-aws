...
- 利用 [SOCI Parallel Pull](https://aws.amazon.com/blogs/containers/introducing-seekable-oci-parallel-pull-mode-for-amazon-eks/){:target="_blank"} 实现快速容器镜像加载
- 在 2 分钟内扩展新的推理节点
...
4. 在Amazon EKS 集群中，Karpenter 会启动新的[Amazon Elastic Compute Cloud](https://aws.amazon.com/ec2/){:target="_blank"} 实例以承载新的副本，这些实例运行 [Bottlerocket OS](https://aws.amazon.com/bottlerocket/){:target="_blank"} 操作系统，采用[Spot](https://aws.amazon.com/ec2/spot)/On-demand混合购买方式，且通过SOCI并行拉取和NVMe实例存储实现快速容器镜像加载（也可选择通过EBS快照预载容器镜像）。
...
您需要为使用该解决方案中包含的AWS服务付费。按2026年3月价格计算，在美国西部（俄勒冈）区域运行该解决方案一个月，且生成一百万张图片的价格约为（不含免费额度）**$430** 美元。
...
|-----------|------------|------------|------------|------------|
| Amazon EC2 | g5.2xlarge 或 g6.2xlarge 实例，Spot 实例每小时  | 416.67 | \$ 0.5300 | \$ 221 |
...
| Amazon EC2 | m7g.large 实例，按需实例每小时费用  | 1440 | \$ 0.0816 | \$ 117.50 |
| **小计，每月** | &nbsp; | &nbsp; | &nbsp; | **\$ 189.50** |

- \* 按每个请求耗时 1.5 秒计算，单价参照 2026年3月美国西部（俄勒冈）区域所有可用区的平均Spot实例价格
- \*\* 按请求平均 16 KB 计算
...
- 在当前版本，该解决方案部署时会自动创建一个新的VPC。该VPC包含：
    ...
    - 1个NAT网关（位于公有子网中）以节省成本
    - 1个Internet网关
    - 对应的路由表和安全组

- 在当前版本，该解决方案部署在新建的EKS集群上，Kubernetes版本为`1.35`。
...
* *（可选）* 创建包含容器镜像的EBS快照进行缓存，或使用默认的SOCI并行拉取+实例存储实现快速启动
...
* `-S, --with-snapshot`: 启用EBS快照创建用于容器镜像缓存。默认：禁用（使用SOCI并行拉取+实例存储）。
...
**注意：** 基于EBS快照的镜像缓存现在是可选的。默认情况下，该方案使用 [SOCI Parallel Pull](https://aws.amazon.com/blogs/containers/introducing-seekable-oci-parallel-pull-mode-for-amazon-eks/) 和NVMe实例存储实现快速容器启动（镜像拉取速度提升可达60%）。如果您更倾向于使用EBS快照缓存，请按照以下说明操作。
...
默认情况下，GPU节点使用 [SOCI Parallel Pull](https://aws.amazon.com/blogs/containers/introducing-seekable-oci-parallel-pull-mode-for-amazon-eks/) 模式和NVMe实例存储实现快速容器启动。此配置通过Helm Chart的默认值自动设置：

- **SOCI 并行拉取**：通过并行下载和解压，加速容器镜像拉取速度最高达60%
- **NVMe 实例存储**：将 `/var/lib/containerd`、`/var/lib/kubelet` 和 `/var/lib/soci-snapshotter` 绑定到高速NVMe磁盘（RAID0）
- **IMDSv2 强制**：强制使用实例元数据服务v2以增强安全性

这些功能在 Bottlerocket GPU 节点上通过 Helm Chart `userData` 配置默认启用。如需禁用，请在运行时的 `extraValues.karpenter.nodeTemplate` 中设置 `userData: ""` 并移除 `instanceStorePolicy`。
...
4. 设置基于 EBS 快照的镜像缓存（可选，使用SOCI并行拉取时不需要）
